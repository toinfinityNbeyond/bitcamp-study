...
//26. 인스턴스 변수로 배열을 관리하기 (이런식으로 코드 안짬)
//      - ArrayList2.java 는 삭제한다.
//      - ArrayList.java의 변수를 인스턴스 변수로 전환한다.
//      - 메서드에서 작업할 때 사용할 변수의 주소(인스턴스 주소)를 파라미터로 받는다.
//      - BoardHandler.java 와 MemberHandler.java 변경
//      - 각 핸들러가 사용할  ArrayList 의 인스턴스 따로 생성한다.
//      -ArrayList 의 메서드를 호출할 때마다 인스턴스 
// 27. 인스턴스 변수를 사용하는 메서드는 인스턴스 메서드로 전환한다.
//      - ArrayList 의 메서드를 클래스 메서드에서 인스턴스 메서드로 전환한다.
//      - BoardHandler 와 MemberhHandler 에서 ArrayList의 메서드를 호출할 때
